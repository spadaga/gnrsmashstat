...
function Logo() {
  ...
  return (
    <span className="group relative shrink-0">
      <img src="/logo.jpeg" alt="GNR SmashStats logo" onError={() => setBroken(true)} className="w-8 h-8 rounded-lg object-cover" />
  ...
  return (
    <header className="bg-white dark:bg-slate-800 border-b dark:border-slate-700 sticky top-0 z-10">
      <div className="max-w-6xl mx-auto px-3 py-2 flex items-center justify-between gap-2">
        {/* Logo */}
        <button onClick={() => nav('dashboard')} className="flex items-center gap-2 text-left shrink-0">
          <Logo />
        ...
        <nav className="hidden md:flex items-center gap-1 flex-wrap flex-1 justify-end">
          {NAV_DESKTOP.filter(({ adminOnly }) => !adminOnly || isAdmin).map(({ key, label, icon: Icon }) => (
            <button key={key} onClick={() => nav(key)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition ${
                page === key ? 'bg-orange-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'
              }`}>
          ...
          <button onClick={onToggleDark} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
          ...
          {isAdmin ? (
            <div className="flex items-center gap-2 ml-1">
              ...
              <button onClick={onLogout} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                <LogOut size={13} /> Logout
              </button>
            </div>
          ) : (
            <button onClick={onLoginClick} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-orange-600 border border-orange-200 dark:border-orange-800 hover:bg-orange-50 dark:hover:bg-orange-900/30 transition ml-1">
            ...
            <>
              <button onClick={() => nav('log')}
                className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-bold transition ${
                  page === 'log' ? 'bg-orange-600 text-white' : 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-800'
                }`}>
                <Plus size={12} /> Log
              </button>
              <span className="text-xs font-semibold text-orange-700 dark:text-orange-400 max-w-[80px] truncate">{adminName}</span>
            </>
          )}
          {!isAdmin && (
            <button onClick={onLoginClick} className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-semibold text-orange-600 border border-orange-200 dark:border-orange-800 hover:bg-orange-50 transition">
              <LogIn size={12} /> Login
            </button>
          )}
          <button onClick={() => setMenuOpen(!menuOpen)} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
      ...
      {menuOpen && (
        <div className="md:hidden border-t dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 space-y-1">
          <button onClick={() => nav('dashboard')} className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition ${page === 'dashboard' ? 'bg-orange-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
            Dashboard
          </button>
          <button onClick={() => nav('players')} className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition ${page === 'players' ? 'bg-orange-600 text-white' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
          ...
          <a href="tel:7569475439" className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
            <Phone size={15} /> Call Bhavani
          </a>
          <div className="flex items-center justify-between px-3 py-2">
            <span className="text-sm text-slate-500 dark:text-slate-400">{dark ? 'Dark Mode' : 'Light Mode'}</span>
            <button onClick={onToggleDark} className="p-1.5 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
              {dark ? <Sun size={16} /> : <Moon size={16} />}
            </button>
          </div>
          {isAdmin && (
            <button onClick={() => { onLogout(); setMenuOpen(false) }} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition">
